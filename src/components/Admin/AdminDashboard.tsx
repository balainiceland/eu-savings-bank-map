...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Top bar */}
      <header className="bg-esb-navy text-white px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Landmark className="w-6 h-6 text-esb-gold" />
          <div>
            <h1 className="font-bold">Admin Dashboard</h1>
            <p className="text-xs text-gray-300">European Savings Bank Map</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={signOut} className="text-sm text-gray-300 hover:text-white flex items-center gap-1">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-2xl font-bold text-esb-navy">{banks.length}</div>
            <div className="text-sm text-gray-500">Total Banks</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-2xl font-bold text-esb-green">{publishedCount}</div>
            <div className="text-sm text-gray-500">Published</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-2xl font-bold text-esb-amber">{draftCount}</div>
        ...
        <div className="flex items-center justify-between mb-4">
            ...
            {(['published', 'draft', 'all', 'settings'] as Tab[]).map(t => (
              <button
                ...
                className={`px-4 py-1.5 text-sm rounded-md font-medium transition-colors ${
                  tab === t ? 'bg-esb-royal text-white' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
          ...
          <div className="flex items-center gap-2">
            <button onClick={loadBanks} className="p-2 hover:bg-gray-200 rounded-lg" title="Refresh">
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button
              onClick={() => setIsCreating(true)}
              className="flex items-center gap-1 px-4 py-2 bg-esb-royal text-white text-sm rounded-lg hover:bg-esb-navy"
            >
          ...
          <>
            ...
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30"
              />
            </div>
            ...
            {loading ? (
              <div className="text-center py-12">
                <div className="w-8 h-8 border-3 border-esb-blue border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
                    ...
                    {filteredBanks.map(bank => (
                      <tr key={bank.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3">
                          <div className="font-medium text-sm text-esb-navy">{bank.name}</div>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => setEditingBank(bank)} className="p-1.5 hover:bg-gray-100 rounded" title="Edit">
                              <Edit2 className="w-4 h-4 text-gray-500" />
                            </button>
                            <button onClick={() => handleToggleFeatured(bank.id, bank.featured)} className="p-1.5 hover:bg-gray-100 rounded" title="Toggle featured">
                              {bank.featured ? <Star className="w-4 h-4 text-esb-gold fill-current" /> : <StarOff className="w-4 h-4 text-gray-300" />}
                            </button>
                            {bank.status === 'draft' ? (
                              <button onClick={() => handlePublish(bank.id)} className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200">
                                Publish
                              </button>
                            ) : (
                              <button onClick={() => handleUnpublish(bank.id)} className="px-2 py-1 text-xs bg-amber-100 text-amber-700 rounded hover:bg-amber-200">
                                Unpublish
                              </button>
                            )}
                            <button onClick={() => handleDelete(bank.id)} className="p-1.5 hover:bg-red-50 rounded" title="Delete">
