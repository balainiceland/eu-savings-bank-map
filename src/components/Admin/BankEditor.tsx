  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-esb-navy">
          {bank ? 'Edit Bank' : 'Add New Bank'}
        </h2>
        <div className="flex items-center gap-2">
          <button type="button" onClick={onCancel} className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">
            <X className="w-4 h-4 inline mr-1" />Cancel
          </button>
          <button type="submit" className="px-4 py-1.5 text-sm bg-esb-royal text-white rounded-lg hover:bg-esb-navy">
      ...
      <fieldset className="border border-gray-200 rounded-lg p-4">
        <legend className="text-sm font-semibold text-esb-navy px-2">Basic Information</legend>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-600 mb-1">Bank Name *</label>
            <input value={name} onChange={e => setName(e.target.value)} required
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={latitude} onChange={e => setLatitude(Number(e.target.value))} type="number" step="any" required
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Longitude *</label>
            <input value={longitude} onChange={e => setLongitude(Number(e.target.value))} type="number" step="any" required
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Founded Year</label>
            <input value={foundedYear || ''} onChange={e => setFoundedYear(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
      ...
      {/* Financial Metrics */}
      <fieldset className="border border-gray-200 rounded-lg p-4">
        <legend className="text-sm font-semibold text-esb-navy px-2">Financial Metrics</legend>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Total Assets (M EUR)</label>
            <input value={totalAssets || ''} onChange={e => setTotalAssets(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Customers (thousands)</label>
            <input value={customerCount || ''} onChange={e => setCustomerCount(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Deposits (M EUR)</label>
            <input value={depositVolume || ''} onChange={e => setDepositVolume(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Loans (M EUR)</label>
            <input value={loanVolume || ''} onChange={e => setLoanVolume(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Employees</label>
            <input value={employeeCount || ''} onChange={e => setEmployeeCount(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Branches</label>
            <input value={branchCount || ''} onChange={e => setBranchCount(e.target.value ? Number(e.target.value) : undefined)} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Reporting Year</label>
            <input value={reportingYear} onChange={e => setReportingYear(Number(e.target.value))} type="number"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-esb-blue/30" />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset className="border border-gray-200 rounded-lg p-4">
        <legend className="text-sm font-semibold text-esb-navy px-2">
          Digital Features (Score: {currentScore}/100)
        </legend>
        <div className="space-y-4">
          {categories.map(category => (
            <div key={category} className="border-b border-gray-100 pb-3 last:border-0">
              <div className="font-medium text-sm text-esb-navy mb-2">{DIGITAL_CATEGORY_LABELS[category]}</div>
              <div className="grid grid-cols-4 gap-2">
                {maturityLevels.map(level => (
                  <label
                    key={level}
                    className={`flex flex-col items-center p-2 rounded-lg border cursor-pointer text-center transition-colors ${
                      featureLevels[category] === level
                        ? 'border-esb-blue bg-blue-50'
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <span className="text-xs font-medium">{MATURITY_LABELS[level]}</span>
      ...
      {/* Status */}
      <fieldset className="border border-gray-200 rounded-lg p-4">
        <legend className="text-sm font-semibold text-esb-navy px-2">Publishing</legend>
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="radio" name="status" value="draft" checked={status === 'draft'}
              onChange={() => setStatus('draft')} className="accent-esb-blue" />
            <span className="text-sm">Draft</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="radio" name="status" value="published" checked={status === 'published'}
              onChange={() => setStatus('published')} className="accent-esb-blue" />
            <span className="text-sm">Published</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer ml-auto">
            <input type="checkbox" checked={featured} onChange={e => setFeatured(e.target.checked)} className="accent-esb-gold" />
